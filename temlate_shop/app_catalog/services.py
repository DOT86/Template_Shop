...
def catalog_obj_order_by(parameter: str, flag: str = None) -> ProductInShop:
    ...
    :param parameter: Передаваемые параметры сортировки (popular, comments, subcategory, category  price, product__created)
    :type parameter: str
    :param flag: Дополнительный параметр сортировки для категории и подкатегрии (subcategory.name, category.name)
    :type flag: str
    :return: Возвращается отсортированный список модели ProductInShop
    :rtype: ProductInShop
    """
...
def get_maxprice(pid: list, maxprice=0):
